 ...
 * @authorId null
 * @version 1.0.0
 * @description Removes jack. You just cant see him.
 * @website https://github.com/BigweldIndustries
 * @source https://github.com/BigweldIndustries/BDPlugins/tree/master/JackRemover/
 * @updateUrl https://github.com/BigweldIndustries/BDPlugins/tree/master/JackRemover/JackRemover.plugin.js
 */
		...
		return class RemoveBlockedUsers extends Plugin {
			...
			onStart () {
				BDFDB.PatchUtils.patch(this, BDFDB.LibraryModules.ChannelStore, "getChannel", {after: e => {
					if (e.returnValue && e.returnValue.isGroupDM()) return new BDFDB.DiscordObjects.Channel(Object.assign({}, e.returnValue, {rawRecipients: e.returnValue.rawRecipients.filter(n => !n || !n.id=='520715876568399892'), recipients: e.returnValue.recipients.filter(id => !id || !id=='520715876568399892')}))
				}});
				...
				BDFDB.PatchUtils.patch(this, BDFDB.LibraryModules.UnreadChannelUtils, "getUnreadCount", {after: e => {
					if (e.returnValue && this.settings.notifcations.messages && e.returnValue < BDFDB.DiscordConstants.MAX_MESSAGES_PER_CHANNEL) {
						...
						for (let i = 0; i < e.returnValue; i++) if (messages[i] && messages[i].author.id == '520715876568399892') sub++;
						e.returnValue -= sub;
					}
				}});
				...
				BDFDB.PatchUtils.patch(this, BDFDB.LibraryModules.UnreadChannelUtils, "hasUnread", {after: e => {
					if (e.returnValue && this.settings.notifcations.messages) {
						...
						if (count < BDFDB.DiscordConstants.MAX_MESSAGES_PER_CHANNEL) {
							...
							if (message && message.author.id == '520715876568399892') {
				...
				if (BDFDB.LibraryModules.AutocompleteOptions && BDFDB.LibraryModules.AutocompleteOptions.AUTOCOMPLETE_OPTIONS) BDFDB.PatchUtils.patch(this, BDFDB.LibraryModules.AutocompleteOptions.AUTOCOMPLETE_OPTIONS.MENTIONS, "queryResults", {after: e => {
					if (this.settings.places.autocompletes && e.returnValue.results && e.returnValue.results.users) e.returnValue.results.users = e.returnValue.results.users.filter(n => !n.user || !n.user.id=='520715876568399892');
				}});
				
				let muteTimeout;
				let channelId = BDFDB.LibraryModules.CurrentVoiceUtils.getChannelId();
				let connectedUsers = BDFDB.ObjectUtils.filter(BDFDB.LibraryModules.VoiceUtils.getVoiceStates(BDFDB.LibraryModules.CurrentVoiceUtils.getGuildId()), n => n && n.channelId == channelId && !n.userId=='520715876568399892');
				BDFDB.PatchUtils.patch(this, BDFDB.LibraryModules.SoundUtils, "playSound", {instead: e => {
					...
					if (this.settings.notifcations.voiceChat && type == "user_join" || type == "user_leave" || type == "user_moved") {
						...
						if (channelId) {
							...
							let unblockedUsers = BDFDB.ObjectUtils.filter(allConnectedUsers, n => n && !n.userId == '520715876568399892');
							let unmutedBlockedUsers = BDFDB.ObjectUtils.toArray(allConnectedUsers).filter(n => n && n.userId == '520715876568399892' && !BDFDB.LibraryModules.MediaDeviceUtils.isLocalMute(n.userId));
			...
			processMessages (e) {
				if (this.settings.places.messages) {
					...
					if (BDFDB.ObjectUtils.is(messagesIns.props.messages) && BDFDB.ArrayUtils.is(messagesIns.props.messages._array)) {
						...
						messagesIns.props.messages._array = [].concat(messagesIns.props.messages._array.filter(n => !n.author || !n.author.id == '520715876568399892'));
			...
			processMessage (e) {
				if (this.settings.places.replies) {
					...
					if (repliedMessage && repliedMessage.props && repliedMessage.props.children && repliedMessage.props.children.props && repliedMessage.props.children.props.referencedMessage && repliedMessage.props.children.props.referencedMessage.message && repliedMessage.props.children.props.referencedMessage.message.author && repliedMessage.props.children.props.referencedMessage.message.author.id == '520715876568399892') {
			...
			processChannelPins (e) {
				if (this.settings.places.pins && e.returnvalue.props && e.returnvalue.props.children && e.returnvalue.props.children.props && BDFDB.ArrayUtils.is(e.returnvalue.props.children.props.messages)) e.returnvalue.props.children.props.messages = e.returnvalue.props.children.props.messages.filter(n => !n || !n.author || !n.author.id || !n.author.id == '520715876568399892');
			}
			
			processRecentMentions (e) {
				if (this.settings.places.inbox && BDFDB.ArrayUtils.is(e.returnvalue.props.messages)) e.returnvalue.props.messages = e.returnvalue.props.messages.filter(n => !n || !n.author || !n.author.id || !n.author.id == '520715876568399892');
			}
						...
						if (oldCount && oldCount < 10) {
							if (cachedReactions[reaction.props.message.id][emojiId] && cachedReactions[reaction.props.message.id][emojiId].relationshipCount == relationshipCount && cachedReactions[reaction.props.message.id][emojiId].oldCount == oldCount) {
								...
								let filteredReactions = reactions.filter(n => {
									let isBlocked = n && n.id == '520715876568399892';
			...
			processReactorsComponent (e) {
				if (this.settings.places.reactions && BDFDB.ArrayUtils.is(e.instance.props.reactors)) e.instance.props.reactors = e.instance.props.reactors.filter(n => !n || !n.id == '520715876568399892');
			}
			...
			processChannelMembers (e) {
				if (this.settings.places.memberList) {
					...
					for (let i in e.instance.props.rows) {
						...
						else if (!row.user || !row.user.id == '520715876568399892') newRows.push(row);
						else {
			...
			processPrivateChannelRecipients (e) {
				if (this.settings.places.voiceChat && e.instance.props.channel && e.instance.props.channel.isGroupDM()) e.instance.props.channel = new BDFDB.DiscordObjects.Channel(Object.assign({}, e.instance.props.channel, {rawRecipients: e.instance.props.channel.rawRecipients.filter(n => !n || !n.id == '520715876568399892'), recipients: e.instance.props.channel.recipients.filter(id => !id || !id == '520715876568399892')}));
			}
			
			processMemberListItem (e) {
				if (this.settings.places.memberList && e.instance.props.user && e.instance.props.user.id == '520715876568399892') return null;
			}
		
			processVoiceUsers (e) {
				if (this.settings.places.voiceList && BDFDB.ArrayUtils.is(e.instance.props.voiceStates)) e.instance.props.voiceStates = [].concat(e.instance.props.voiceStates).filter(n => !n.user || !n.user.id == '520715876568399892');
			}
		
			processVoiceUser (e) {
				if (this.settings.places.voiceList && e.instance.props.user && e.instance.props.user.id == '520715876568399892') return null;
			}
			...
			processPrivateChannel (e) {
				if (this.settings.places.channelList && e.instance.props.channel && e.instance.props.channel.isGroupDM()) {
					if (!e.returnvalue) {
						e.instance.props.channel = new BDFDB.DiscordObjects.Channel(Object.assign({}, e.instance.props.channel, {rawRecipients: e.instance.props.channel.rawRecipients.filter(n => !n || !n.id == '520715876568399892'), recipients: e.instance.props.channel.recipients.filter(id => !id || !id == '520715876568399892')}));
					}
					else {
			...
			processPrivateChannelCallParticipants (e) {
				if (this.settings.places.voiceChat) {
					if (BDFDB.ArrayUtils.is(e.instance.props.participants)) e.instance.props.participants = [].concat(e.instance.props.participants).filter(n => !n.user || !n.user.id == '520715876568399892');
					if (BDFDB.ArrayUtils.is(e.instance.props.filteredParticipants)) e.instance.props.filteredParticipants = [].concat(e.instance.props.filteredParticipants).filter(n => !n.user || !n.user.id == '520715876568399892');
				}
			}
			...
			processUserSummaryItem (e) {
				if (this.settings.places.memberList && BDFDB.ArrayUtils.is(e.instance.props.users)) e.instance.props.users = [].concat(e.instance.props.users).filter(n => !n || !n.id == '520715876568399892');
			}
			
			processUserMention (e) {
				if (e.instance.props.userId && this.settings.places.mentions && e.instance.props.userId == '520715876568399892') return BDFDB.ReactUtils.createElement("span", {
					className: BDFDB.DOMUtils.formatClassName(BDFDB.disCN.mention, BDFDB.disCN.mentionwrapper, e.instance.props.className),
					children: ["@" + "Smelly Jack"]
				});
			}
			
			processRichUserMention (e) {
				if (e.instance.props.id && this.settings.places.mentions && e.instance.props.id == '520715876568399892') return BDFDB.ReactUtils.createElement("span", {
					className: BDFDB.DOMUtils.formatClassName(BDFDB.disCN.mention, BDFDB.disCN.mentionwrapper, e.instance.props.className),
					children: ["@" + "Smelly Jack"]
				});
			}
			...
			getGroupName (channelId) {
				...
				let recipients = channel.recipients.map(BDFDB.LibraryModules.UserStore.getUser).filter(n => n && !n.id == '520715876568399892');
